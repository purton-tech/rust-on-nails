...
static LICENCE_SECRET: &str = "nails-licence";

pub const APPLICATION_NAME: &str = "nails-app";
static CONFIG_MAP_NAME: &str = "application-config";
...
pub async fn deploy(client: Client, spec: NailsAppSpec, namespace: &str) -> Result<(), Error> {
    let mut env = vec![
        ...
        json!({
            ...
            "LOGOUT_URL",
            "value":
            "/oidc/realms/nails/protocol/openid-connect/logout"
        }),
        json!({
            "name": "APP_BASE_URL",
...
async fn licence_secret(namespace: &str, client: Client) -> Result<(), Error> {
    ...
    if secret.is_err() {
        ...
        secret_data.insert(
            LICENCE.to_string(),
            "Contact support for a licence.".to_string(),
        );
...
async fn email_secret(namespace: &str, spec: NailsAppSpec, client: Client) -> Result<(), Error> {
    let secret_api: Api<Secret> = Api::namespaced(client, namespace);
...
pub async fn delete(client: Client, namespace: &str) -> Result<(), Error> {
    ...
    if api.get(APPLICATION_NAME).await.is_ok() {
        api.delete(APPLICATION_NAME, &DeleteParams::default())
            .await?;
    }
