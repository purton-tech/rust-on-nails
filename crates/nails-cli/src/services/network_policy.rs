use super::{
    application::APPLICATION_NAME, keycloak::KEYCLOAK_NAME, rag_engine::NAME as RAG_ENGINE_NAME,
};
...
pub async fn default_deny(client: Client, name: &str, namespace: &str) -> Result<(), Error> {
    ...
    let egress = if name == APPLICATION_NAME || name == KEYCLOAK_NAME || name == RAG_ENGINE_NAME {
        json!([
